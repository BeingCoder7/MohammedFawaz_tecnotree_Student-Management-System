...
def exam_results():
    if session.get('logged_in'):
        try:
            with mydb.cursor() as cursor:
                sql = "SELECT * FROM exam_results_view"
                cursor.execute(sql)
     ...
     if session.get('logged_in'):
        if request.method == 'POST': 
            try:
                ...
                with mydb.cursor() as cursor:
                    sql = "INSERT INTO exam_results (student_id, course_id, exam_id, marks_obtained, total_marks) VALUES (%s, %s, %s, %s, %s)"
                    cursor.execute(sql, (student_id, course_id, exam_id, marks_obtained, total_marks))
                    mydb.commit()
                    return "Exam result added successfully"
            except:
                return "Error adding exam result"
        else:
